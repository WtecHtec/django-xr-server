...
def get_index_vector():
    ...
    print('index.ntotal last ===',index.ntotal)
    total_dataset_dtb = get_total_dataset_dtb()
    print('total_dataset_dtb===', total_dataset_dtb)
    vector_total = 0
    ...
    print('index.ntotal nesw ===',index.ntotal)
    return index
...
def get_image_similar(path_image):
    ...
    print('result_vector===', len(result_vector), result_vector[0])
    query = ""
    for j in range(total_results):
        if result_vector[0][j] > -1 :
